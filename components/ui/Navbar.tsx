...
export default async function Navbar() {
  ...
  return (
    <header className="shrink-0 bg-ink flex flex-col sticky top-0 z-50">
      ...
      <div className="bg-cream px-4 py-5 sm:py-8">
      ...
      <nav className="text-cream-warm">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 flex items-center gap-0 flex-wrap">
          ...
          {user && (
            ...
            <Link
              href="/mensajes"
              className="text-[11px] sm:text-xs font-medium uppercase tracking-[0.1em] sm:tracking-[0.15em] px-2.5 sm:px-4 py-5 sm:py-7 hover:bg-brand-500/10 hover:text-brand-400 transition-colors whitespace-nowrap flex items-center"
            >
              Mensajes
              <UnreadBadge />
            </Link>
          ...
          <a
            ...
            className="ml-auto text-[11px] sm:text-xs font-medium uppercase tracking-[0.1em] sm:tracking-[0.15em] px-2.5 sm:px-3 py-5 sm:py-7 hover:bg-white/10 transition-colors whitespace-nowrap flex items-center gap-1.5 text-green-400 hover:text-green-300"
...
function NavLink({ href, children, className = "" }: { href: string; children: React.ReactNode; className?: string }) {
  return (
    <Link
      href={href}
      className={`text-[11px] sm:text-xs font-medium uppercase tracking-[0.1em] sm:tracking-[0.15em] px-2.5 sm:px-4 py-5 sm:py-7 hover:bg-white/10 transition-colors whitespace-nowrap inline-flex items-center ${className}`}
    >
      {children}
    </Link>
  );
