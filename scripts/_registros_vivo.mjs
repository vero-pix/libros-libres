...
const {data:us}=await s.from("users").select("id,full_name,username,email,created_at,city,mercadopago_user_id").gte("created_at",since).order("created_at",{ascending:false});
...
for(const u of (us??[])){
  ...
  console.log(`     vistas:${pv} libros:${lst} ciudad:${u.city??"—"} MP:${u.mercadopago_user_id?"✅":"—"}`);
}
