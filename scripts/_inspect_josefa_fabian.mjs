...
const { data: users, error } = await s.from("users")
  .select("id,full_name,email,username,city,featured,mercadopago_user_id,created_at")
  .in("id",ids);
if(error){console.log("ERROR:",error.message);process.exit(1);}
const now=Date.now();
const newNoMp = users.filter(u=> u.created_at && (now-new Date(u.created_at).getTime())<30*86400000 && !u.mercadopago_user_id);
