...
for(const un of ["lorena.cortes","nicolas"]){
  const {data:u}=await s.from("users").select("full_name,featured,city,mercadopago_user_id,mercadopago_connected_at").eq("username",un).maybeSingle();
  console.log(`@${un}: featured=${u.featured} · MP=${u.mercadopago_user_id?"✅ conectado "+(u.mercadopago_connected_at??""):"❌ no"} · ciudad=${u.city??"—"}`);
}
