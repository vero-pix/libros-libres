...
const {data:u}=await s.from("users").select("id,full_name,email,city,featured,mercadopago_user_id").eq("username","sol.pg").maybeSingle();
console.log(`${u.full_name} · ${u.email} · ${u.city} · feat=${u.featured} · MP=${u.mercadopago_user_id?"sí":"no"}`);
