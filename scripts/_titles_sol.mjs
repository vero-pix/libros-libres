...
const {data:u}=await s.from("users").select("id,full_name,email,city,featured,mercadopago_access_token").eq("username","sol.pg").maybeSingle();
console.log(`${u.full_name} · ${u.email} · ${u.city} · feat=${u.featured} · MP=${u.mercadopago_access_token?"sí":"no"}`);
