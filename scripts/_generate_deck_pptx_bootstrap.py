...
# Paleta CLARITA — sin fondos oscuros, apta para audiencia familiar
NAVY = RGBColor(0xed, 0xe0, 0xc1)        # cream cálido saturado — reemplaza navy
NAVY_LIGHT = RGBColor(0xf3, 0xea, 0xd5)  # cream dorado claro
GOLD = RGBColor(0xa7, 0x7a, 0x0e)        # dorado oscuro, contraste sobre cream
GOLD_SOFT = RGBColor(0xd4, 0xa0, 0x17)   # dorado clásico
CREAM = RGBColor(0xfa, 0xf7, 0xf1)
INK = RGBColor(0x2d, 0x2e, 0x3d)         # tinta suave, no negro
MUTED = RGBColor(0x8b, 0x8b, 0x9c)
WHITE = RGBColor(0xff, 0xff, 0xff)
DEEP_BLUE = RGBColor(0x3d, 0x46, 0x70)   # azul suave para títulos
...
add_text(s, "tuslibros.cl", Inches(0.6), Inches(2.3),
         ...
         font=FONT_SERIF, size=92, bold=False, color=INK)
...
add_text(s, "Los libros de tu ciudad.\nMarketplace de libros usados con mapa, curación y split payment.",
         ...
         font=FONT_SANS, size=20, color=INK)
...
add_text(s, "Un marketplace con tres diferenciadores\nque nadie tiene junto.",
         Inches(0.6), Inches(1.0), Inches(12), Inches(1.8),
         font=FONT_SERIF, size=38, color=DEEP_BLUE)
...
add_text(s, "Sin distribución activa ni gasto en marketing.",
         Inches(0.6), Inches(1.0), Inches(12), Inches(0.9),
         font=FONT_SERIF, size=34, color=DEEP_BLUE)
...
add_text(s, "Operación iniciada 3 abril 2026. Tres ventas reales en 18 días: Zdravko (La Marina Tomo I, $5.000), set de 7 Maigret a tercer comprador ($40.000), bundle multi-libro a Camilo (17 abril, Concepción, $45.000).",
         ...
         font=FONT_SANS, size=9, italic=True, color=MUTED)
add_page_num(s, "05 / 11", color=MUTED)
...
definition_box.fill.fore_color.rgb = RGBColor(0xf3, 0xea, 0xd5)
...
for i, (num, lbl, desc) in enumerate(market):
    ...
    add_text(s, lbl.upper(), left, Inches(4.15), Inches(3.9), Inches(0.5),
             font=FONT_SANS, size=11, bold=True, color=MUTED)
...
add_page_num(s, "07 / 11", color=MUTED)
...
add_text(s, "No es limitación: es el modelo. Dev asistido por Claude Code me permite iterar producción en horas lo que una startup tradicional iteraría en semanas.",
         ...
         font=FONT_SANS, size=13, italic=True, color=INK)
...
add_text(s, "El argumento tradicional contra un founder único es ejecución. La evidencia de los últimos 18 días responde eso antes de la pregunta.",
         Inches(0.6), Inches(6.95), Inches(12), Inches(0.4),
         font=FONT_SANS, size=10, italic=True, color=MUTED)
add_page_num(s, "09 / 11", color=MUTED)
...
# ═══════════════════════════════════════════════════════════════
s = blank_slide(CREAM)
add_tag(s, "Propuesta · Bootstrap")
add_text(s, "USD 40-60k · 3-5% · pre-money USD 1.2M.",
         Inches(0.6), Inches(1.0), Inches(12), Inches(0.7),
         font=FONT_SERIF, size=34, color=INK)
add_text(s, "$38 – $57 millones CLP  ·  ronda ángel  ·  SAFE o nota convertible  ·  12 meses opex puro",
...
# Columna derecha — uso de fondos BOOTSTRAP
add_text(s, "USO DE FONDOS · 12 MESES  (USD 50k base)", Inches(6.9), Inches(2.45),
         Inches(6), Inches(0.4),
         font=FONT_SANS, size=11, bold=True, color=GOLD)

funds_rows = [
    ("Marketing digital + GTM (Kit Metralleta)", "45%"),
    ("Activación librerías + dev apoyo", "30%"),
    ("Features clave (mobile, reseñas)", "15%"),
    ("Infra + reserva operativa", "10%"),
]
...
add_text(s, "Founder subsidia su tiempo con ahorros. 100% del capital va a ejecución de mercado. Break-even mes 14 en escenario bull.",
...
hitos = [
    ...
    "Listo para ronda seed USD 500k-1M (donde sí va salario)",
]
...
add_text(s, "El mejor momento para",
         ...
         font=FONT_SERIF, size=60, color=INK)
...
add_page_num(s, "11 / 11", color=MUTED)


# Guardar
output = '/Users/veronicavelasquez/libros-libres/docs/deck_valorizacion_bootstrap.pptx'
prs.save(output)
print(f"✓ Generado: {output}")
print(f"  {len(prs.slides)} slides, 16:9 widescreen (versión BOOTSTRAP sin salario founder)")
