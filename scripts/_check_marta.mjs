...
const { data: u } = await sb.from('users').select('id, full_name, username, city, featured, mercadopago_user_id, created_at, email').ilike('full_name', '%yllades%');
console.log('=== USUARIA ===');
for (const x of u) console.log({ id: x.id.slice(0,8), name: x.full_name, user: x.username, city: x.city, featured: x.featured, mp: !!x.mercadopago_user_id, email: x.email });
