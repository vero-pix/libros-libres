...
- **Landings antes que fichas — pero ojo con qué landing.** Las fichas de libro casi no rankean solas (795 fichas = 285 clics/90d; 79 landings = 2.327). ⚠️ **Verificado el 17-09-2026: esos 2.327 son la home (1.903) y las landings GEO** (concepción 104, temuco 50, valparaíso 44…). Las landings **temáticas** están en cero: `/antroposofia` 0 clics, `/filosofia` 0, `/espiritualidad` 0 impresiones — aunque solo tenían 13 días, y el SEO tarda de 4 a 12 semanas. **No prometer que una landing de tema va a traer tráfico**; lo demostrado acá son las geo con intención vendedora. Para posicionar un autor: entrada en `app/(main)/autor/[slug]/authors.config.ts` + match en `lib/authorLandings.ts`. Para un tema: copiar `app/(main)/antroposofia/page.tsx` y agregarla a `app/sitemap.ts` y `app/llms.txt/route.ts`. Para raros/caros: marcar `is_collectible`, que `/libros-antiguos` los muestra — y si es un **impreso chileno**, además el tag `impreso-chileno` en `books.tags`, que lo sube a la sección propia de esa landing y al filtro `/?tag=impreso-chileno`. **`is_collectible` está diluido** (58 activos, 26 son Simenon de $7.990): no es sinónimo de pieza. Y que las fichas enlazadas tengan sinopsis real: la plantilla "Descubre X de Y. Este libro usado…" en `books.description` deja la meta description vacía.
