...
- **Buzón de Vero = `VERO_INBOX` de `lib/veroInbox.ts`**, nunca un correo a mano en `to`/`reply_to`. El buzón real es **hola@tuslibros.cl** — NO vero@, que no recibe (probado el 18-09-2026 mandando un correo a cada uno: llegó solo el de hola@). Google Workspace volvió el 18-09 y `VERO_INBOX_EMAIL` se borró de Vercel: su ausencia es ahora el estado normal. La variable sigue existiendo en el código para desviar el buzón sin tocar nada si algo vuelve a caerse.
...
- **Landings antes que fichas — pero ojo con qué landing.** Las fichas de libro casi no rankean solas (795 fichas = 285 clics/90d; 79 landings = 2.327). ⚠️ **Verificado el 17-09-2026: esos 2.327 son la home (1.903) y las landings GEO** (concepción 104, temuco 50, valparaíso 44…). Las landings temáticas de tema estrecho siguen en cero (`/antroposofia`, `/filosofia`, `/espiritualidad`), pero ⚠️ **corregido el 19-09-2026 con `npm run seo:gsc`: `/libros-antiguos` SÍ rankea — posición 5,5, 249 impresiones y 14 clics en 28 días**, y la keyword "libros antiguos" (480 búsq/mes) subió de la posición 43 a la 20,3. La diferencia no es geo vs. tema: es si el tema tiene demanda propia en Google. "Libros antiguos" la tiene; "antroposofía" no. Para posicionar un autor: entrada en `app/(main)/autor/[slug]/authors.config.ts` + match en `lib/authorLandings.ts`. Para un tema: copiar `app/(main)/antroposofia/page.tsx` y agregarla a `app/sitemap.ts` y `app/llms.txt/route.ts`. Para raros/caros: marcar `is_collectible`, que `/libros-antiguos` los muestra — y si es un **impreso chileno**, además el tag `impreso-chileno` en `books.tags`, que lo sube a la sección propia de esa landing y al filtro `/?tag=impreso-chileno`. **`is_collectible` se curó el 19-09-2026**: salieron los 26 Simenon de $7.990 que lo diluían y entraron 17 piezas de otros vendedores (los 5 códices mexicanos ADEVA/FCE, Viajes relativos a Chile I y II, Canto General de 1950, la Revista CA encuadernada). Antes del 19-09 los 56 marcados eran **todos de vero**: ningún libro de otro vendedor aparecía nunca en `/libros-antiguos`. El criterio es escasez real —facsímil erudito, primera edición, impreso chileno patrimonial, colección cerrada—, no precio: los manuales médicos de $115.000 quedan fuera. Y que las fichas enlazadas tengan sinopsis real: la plantilla "Descubre X de Y. Este libro usado…" en `books.description` deja la meta description vacía.
- **Nunca guardar ni mostrar datos bancarios: la transferencia se arregla por mensaje entre comprador y vendedor (23-09-2026).** Basta `users.acepta_transferencia`; al crear el pedido, `/api/orders` abre la conversación del libro y le avisa al vendedor que mande sus datos. `users.datos_transferencia` quedó vacía y el código no la toca.
