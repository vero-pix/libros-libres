      ...
      if (status === "paid") {
        ...
        try {
          ...
          if (fullOrders && fullOrders.length > 0) {
            ...
            if (buyerEmail) {
              // Plantilla en lib/order-emails.ts: así se puede renderizar y
              // probar sin fingir un pago.
              const m = correoCompradorCompraConfirmada({
                buyerName,
                sellerName,
                sellerId: first.seller?.id ?? null,
                itemsRows,
                itemCount,
                bundleTotal,
                deliveryMethod,
